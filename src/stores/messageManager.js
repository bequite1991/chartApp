...
class MessageManager extends EventEmitter{
        ...
        if(!this.isCmdEmpty()){
            this.cmdList.forEach(cmd => {
                ...
                if(this.hasTopic(cmd)){
                    ...
                    let topic_custom = PROTOCAL_REQUEST[cmd].split("/")[4];
                    let username = this.serverOptionsValue.userName;
                    let message_num = "0001";
                    let message = this.packetMessage(cmd,topic_custom,username,this.timestamp,message_num);
    ...
    packetMessage(cmd,topic_custom,username,timestap,message_num){
    let key = md5 (username + 'inshn_gwapi');
    let token = Base64.encode ({cmd: cmd, key: key, timestamp: timestap});
    let messageIndex = 0;
    let num_id = timestampToTime(timestap) + message_num;
